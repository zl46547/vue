...
(function (global, factory) {
  ...
  // 对于普通HTML标记，可以完全跳过标准化，因为生成的呈现函数保证返回数组。有两种情况需要额外的标准化:
  ...
  // 1. 当子组件包含组件时，因为函数组件可能返回一个数组而不是一个根节点。在这种情况下，只需要一个简单的标准化，
  // 如果任何子元素是一个数组，那么我们就用Array.prototype.concat将数组打平。它被保证只有1层的深度，
  // 因为功能组件已经将它们自己的子组件标准化了。
  /**
